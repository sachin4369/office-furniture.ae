...
const getVisitorChairData = async (id) => {
  let data = await fetch(`http://localhost:3000/api/visitorapi/${id}`);
  data = await data.json();
  return data.result;
};

const Page = async ({ params }) => {
  const visitor = await getVisitorChairData(params.productid);
  return (
    <>
      ...
      <div className="w-72">
        ...
        <div className="flex flex-col justify-between leading-normal pl-0 md:pl-20">
          ...
          <div className="font-normal text-sm text-gray-700 space-y-2">
            <p>Model No. : {visitor.ModelNo}</p>
            <p>Type : {visitor.Type}</p>
            <p>Fabric : {visitor.Fabric}</p>
            <p>Mechanism : {visitor.Mechanism}</p>
            <p>Armrest : {visitor.Armrest}</p>
            <p>Mechanism : {visitor.Mechanism}</p>
            <p>BodyColor : {visitor.BodyColor}</p>
            <p>Base Material : {visitor.BaseMaterial}</p>
            <p>Height Adjuster : {visitor.HeightAdjuster}</p>
