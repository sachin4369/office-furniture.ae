...
const getVisitorChairData = async () => {
  let data = await fetch("http://localhost:3000/api/visitorapi");
  data = await data.json();
  return data;
}

const VisitorChairSeries = async () => {
  const visitorChair = await getVisitorChairData();
  return (
    <>
      ...
      <h1 className="text-3xl font-semibold my-5">Visitor Chairs</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {visitorChair.map((item) => (
          <div
            key={item.id}
          ...
          >
            ...
            <Link
              href={`/products/visitor-chair-series/${item.id}`}
...
export default VisitorChairSeries