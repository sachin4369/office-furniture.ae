...
const getCafeChairData = async (id) =>{
    let data = await fetch(`http://localhost:3000/api/cafeapi/${id}`);
    data = await data.json();
    return data.result;
}

const page = async ({params}) => {
    const cafe = await getCafeChairData(params.productid);
    console.log(cafe);
  ...
  return (
    <>
      ...
      <div className="flex flex-col items-center pl-0 md:pl-20 bg-white border border-gray-200 rounded-lg shadow md:flex-row dark:border-gray-700 dark:bg-gray-800 p-4">
        <div className="w-52">
        <Image
        ...
        <div className="flex flex-col justify-between leading-normal pl-0 md:pl-20">
          ...
          <div className="font-normal text-sm text-gray-700 space-y-2">
            <p>
                Model No. : {cafe.ModelNo}
            </p>
            <p>
                Type : {cafe.Type}
            </p>
            <p>
                Frame Material : {cafe.FrameMaterial}
            </p>
            <p>
                Height Adjuster : {cafe.HeightAdjuster}
            </p>
