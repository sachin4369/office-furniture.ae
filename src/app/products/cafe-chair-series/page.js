...
const CafeChairSeries = async () => {
    const cafeChair = await getCafeChairData();
    return(
        <>
      ...
      <h1 className="text-3xl font-semibold my-5">Cafe Chairs</h1>
        ...
        {cafeChair.map((item) => (
          ...
          >
            ...
            <p className="font-normal text-gray-700 space-y-1 flex flex-col mb-4 text-sm">
              ...
              <span>{item.Type}</span>
              <span>{item.HeightAdjuster}</span>
            </p>
            <Link
              href={`/products/cafe-chair-series/${item.id}`}
...
export default CafeChairSeries;