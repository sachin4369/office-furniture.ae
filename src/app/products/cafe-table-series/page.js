...
const getCafeTableData = async () => {
    let data = await fetch("http://localhost:3000/api/cafetableapi/");
    data = await data.json();
    return data;
}

const CafeTableSeries = async ()=> {
    const cafeTable = await getCafeTableData();
    return (
        <>
      ...
      <h1 className="text-3xl font-semibold my-5">Cafeteria Table Series</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {cafeTable.map((item) => (
          ...
          >
            ...
            <Link
              href={`/products/cafe-table-series/${item.id}`}
...
export default CafeTableSeries;