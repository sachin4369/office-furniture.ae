...
const getWorkStationChairData = async () =>{
  try {
    ...
    let data = await fetch(`${apiUrl}/api/workstationapi/`)
...
const WorkStationChairSeries = async () => {
    try {
      const workStationChair = await getWorkStationChairData();
    return (
      <>
        ...
        <h1 className="text-3xl font-semibold my-5">Work Station Chairs</h1>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {workStationChair.map((item) => (
            <div
              key={item.id}
            ...
            >
              ...
              <Link
                href={`/products/workstation-chair-series/${item.id}`}
...
export default WorkStationChairSeries;