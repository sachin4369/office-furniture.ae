...
const getMeshChairData = async () => {
  try {
    const apiUrl = process.env.NEXT_PUBLIC_API_URL;
      let data = await fetch(`${apiUrl}/api/meshapi/`);
...
const MeshChairSeries = async () => {
  try {
    const meshChair = await getMeshChairData();
    return (
      <>
        ...
        <h1 className="text-3xl font-semibold my-5">Mesh Chairs</h1>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {meshChair.map((item) => (
            ...
            >
              ...
              <Link
                href={`/products/mesh-chair-series/${item.id}`}
...
export default MeshChairSeries;
